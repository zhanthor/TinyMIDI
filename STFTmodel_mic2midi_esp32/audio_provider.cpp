...
bool AudioProviderReadWindow(float* out, int hop) {
  ...
  // On the first call, fill the entire window.
  ...
  while (filled < want) {
    ...
    if (bytes_read == 0) {
      Serial.println("I2S returned 0 bytes");
      return false;
    }
  ...
  // Serial.println("I2S samples received");

  // Slide the window and append new samples.
  ...
  for (int i = 0; i < want; i++) {
    tail[i] =
      ((float)g_raw[i] / 2147483648.0f) * kMicGain;
  }
  ...
  //   // Print first 10 audio samples for debugging.
  // Serial.print("First samples: ");

  // for (int i = 0; i < 10; i++) {
  //   Serial.print(g_window[i], 6);
  //   Serial.print(" ");
  // }
  ...
  // Serial.print("RMS: ");
  // Serial.println(g_last_rms, 6);

  memcpy(
    out,
    g_window,
    kWindowSamples * sizeof(float)
  );
